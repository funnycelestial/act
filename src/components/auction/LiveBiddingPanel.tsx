...
export const LiveBiddingPanel = ({ auctionType }: LiveBiddingPanelProps) => {
  ...
  const currentBidData = isReverse 
    ? { amount: "1,200", label: "Lowest Quote", starter: "Starting budget: 2,000 tokens" }
    : { amount: "1,250", label: "Current Bid", starter: "Starting bid: 500 tokens" };
        ...
        <div className="w-2/3 pl-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-foreground font-bold">{currentBidData.label}: {currentBidData.amount} Tokens</span>
              {isHighestBidder && (
            ...
            <div className="space-y-1 text-xs max-h-20 overflow-y-auto">
              {mockBids.map((bid, index) => (
                ...
                >
                  ...
                  <span className={bid.status === 'leading' ? 'text-terminal-green' : ''}>{bid.amount} tokens</span>
                </div>
              ))}
            </div>
